...
function complianceFindings(document, today, catalog) {
  ...
  // A policy SELECTS and configures rules; it never invents one. The schema enforces the shape of an
  // id but cannot know which ids exist, so without this a typo — or a rule someone wished into
  // being — is accepted silently and then never evaluated, which reads as compliance rather than as
  // a mistake. This is the policy half of the guard `assertBindings` provides for the evaluator:
  // the catalog defines rule identity, and neither of the other two may redefine it.
  if (catalog) {
    const known = (id) => catalog.rules.has(id) || catalog.aliases.has(id);
    for (const id of Object.keys(document.rules ?? {})) {
      ...
      findings.push({
        ...
        remediation:
          "Correct the id, or remove the declaration. A policy selects and configures catalogued rules; it cannot define new ones.",
      });
    }
    for (const id of Object.keys(document.applicability ?? {})) {
      if (known(id)) continue;
      findings.push({
        id: "policy.unknown-rule",
        severity: "error",
        message: `applicability is declared for '${id}', which is not a rule the catalog defines`,
        remediation: "Correct the id, or remove the declaration.",
      });
    }
  }
